...
def _preprocess_for_heat_map(
    ...
    if class_list:
        ...
        if len(label_frequency_dict) > label_for_display:
            ...
            workspace_subsampled = workspace_df[
...
def _get_counts_per_label(training_data, unigrams_col_name="unigrams"):
    ...
    """
    training_data[unigrams_col_name] = training_data["utterance"].apply(
        nltk.word_tokenize
    )
    rows = list()
...
def _get_top_n(series, top_n=5, index_level=0):
    ...
    """
    return (
        series.groupby(level=index_level)
        .nlargest(top_n)
...
def seaborn_heatmap(
    ...
    table_format = (
        table_format[table_format.index.isin(most_frequent_words)]
        .fillna(0)
        .astype("int32")
    )
    display(
        Markdown('## <p style="text-align: center;"> Token Frequency per Intent </p>')
    )
    fig, ax = plt.subplots(figsize=(20, 20))

    sns.heatmap(table_format, annot=True, fmt="d", linewidths=0.1, cmap="PuBu", ax=ax)
