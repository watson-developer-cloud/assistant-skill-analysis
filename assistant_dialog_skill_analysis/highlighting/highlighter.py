...
def get_highlights_in_batch_multi_thread(conversation,
                                         workspace_id,
                                         full_results,
                                         output_folder,
    ...
    display(Markdown("### Identified {} problematic utterances "
                     .format(len(wrong_examples_sorted))))
    display(Markdown("  "))

    wrong_examples_sorted = wrong_examples_sorted[: show_worst_k]

    adversarial_results, adversarial_span_dict = _adversarial_examples_multi_thread_inference(
        ...
        if item['correct_intent'] == skills_util.OFFTOPIC_LABEL:
            if item['top_confidence'] > confidence_threshold:
                ...
                highlighting_candidates.append(
                    (idx, item['utterance'], None, item['top_intent'],
                     item['top_confidence'], rank_score, reference_position))
        else:
            if(item['top_intent'] != item['correct_intent']) or (
                ...
                if item['top_intent'] != item['correct_intent']:
                    ...
                    if item['correct_intent'] not in result_dict:
                        ...
                        rank_score = item['top_confidence'] - result_dict[item['correct_intent']]
                ...
                highlighting_candidates.append(
                    (idx, item['utterance'], item['correct_intent'],
                     item['top_intent'], item['top_confidence'],
                     rank_score, reference_position))

    highlighting_candidates_sorted = sorted(highlighting_candidates,
                                            key=lambda x: x[5], reverse=True)
    highlighting_candidates_sorted = [candidate for candidate in highlighting_candidates_sorted
...
def _plot_highlight(highlight, original_example, output_folder):
    ...
    ax = sns.heatmap([[i] for i in highlight.tolist()],
    ...
    if output_folder:
        ...
        if original_example[2]:
            filename = str(original_example[0]) + '_groundtruth_' + \
                original_example[2] + '_prediction_' + \
                original_example[3] + '_confidence_' + \
                conf_str + '.png'
        else:
            filename = str(original_example[0]) + '_groundtruth_offtopic_prediction_' + \
                original_example[3] + '_confidence_' + \
                conf_str + '.png'
    ...
    table.append(
        ['Actual Intent', original_example[2] if (
            original_example[2]) else skills_util.OFFTOPIC_LABEL])
...
def _adversarial_examples_multi_thread_inference(
    ...
    for original_example in wrong_examples_sorted:
        ...
        all_adversarial_label_idx.extend([adversarial_label] * len(adversarial_examples))
    ...
    adversarial_results = inferencer.inference(conversation, workspace_id,
                                               adversarial_test_data_frame, max_retries=10,
                                               max_thread=5, verbose=False)
    display(Markdown("   "))
...
def _highlight_scoring(
    ...
    for idx in range(len(subset_adversarial_result)):
        ...
        start, end = adversarial_span_dict[adversarial_example['utterance'] +
                                           '_' + str(original_idx)]

        highlight = _scoring_function(highlight,
                                      original_position,
...
def _scoring_function(highlight,
                      original_position,
                      adversarial_position,
                      original_confidence,
                      adversarial_confidence,
    ...
    weighted_difference = weight * (
        (0.2 * confidence_difference) +
        (0.8 * position_difference)) / ngram_size
