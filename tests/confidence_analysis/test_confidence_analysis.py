...
from assistant_skill_analysis.confidence_analysis.confidence_analyzer import (
    ...
    _find_threshold,
    abnormal_conf,
    generate_unique_thresholds,
    _get_bot_coverage_list,
    analysis,
    extract_table_analysis,
...
class TestThresholdAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.results = pd.DataFrame(
            self.sorted_list, columns=["correct_intent", "top_intent", "top_confidence"]
        )
    ...
    def test_abnormal_conf(self):
        test_dataframe = pd.DataFrame(
            {
                "correct_intent": ["A", "A", "A"],
                "top_intent": ["A", "B", "B"],
                "top_confidence": [0.2, 0.9, 0.9],
                "utterance": ["a", "a", "a"],
                "top_predicts": [
                    [],
                    [],
                    [
                        ...
                        {"intent": "C", "confidence": 0.1},
                    ],
                ],
            }
        )

        correct_low_conf, incorrect_high_conf = abnormal_conf(test_dataframe, 0.3, 0.7)
        self.assertEqual(len(correct_low_conf), 1, "test_find_threshold failed: first")
        self.assertEqual(
            len(incorrect_high_conf), 2, "test_find_threshold failed: first"
        )
        self.assertEqual(
            incorrect_high_conf.iloc[0, :]["top2_prediction"],
            "NA",
    ...
    def test_get_bot_coverage_list(self):
        ...
        for r, g in zip(res, gt):
            self.assertEqual(
                math.isclose(r, g, abs_tol=0.01, rel_tol=0.0001),
                True,
    ...
    def test_analysis(self):
        ...
        self.assertEqual(
            analysis_df1["Bot Coverage Counts"].iloc[9], "1 / 9", "analysis fail"
        )
        analysis_df_list = analysis(self.results, ["A"])
        self.assertEqual(
            analysis_df_list[0]["Bot Coverage Counts"].iloc[9], "2 / 7", "analysis fail"
        )
    ...
    def test_analysis_pipeline(self):
        ...
        self.assertEqual(
            analysis_df["Bot Coverage Counts"].iloc[9], "1 / 9", "analysis fail"
        )
    ...
    def test_extract_table_analysis(self):
        ...
        self.assertEqual(
            math.isclose(toa_list[2], 0.75, abs_tol=0.01, rel_tol=0.0001),
            True,
    ...
    def test_create_display_table(self):
        ...
        self.assertEqual(
            analysis_df["Bot Coverage Counts"].iloc[-1],
            "1 / 9",
            "create display \
                         table test fail",
        )
    ...
    def test_generate_unique_thresholds(self):
        ...
        self.assertEqual(
            math.isclose(thresholds[1], 0.15, abs_tol=0.01, rel_tol=0.0001),
            True,
            "test \
        ...
        self.assertEqual(
            math.isclose(unique_confidence[5], 1, abs_tol=0.01, rel_tol=0.0001),
            True,
