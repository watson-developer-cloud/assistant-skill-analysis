...
class TestChi2Analyzer(unittest.TestCase):
    ...
    def test_preprocess_chi2(self):
        ...
        labels, convec, features = chi2_analyzer._preprocess_chi2(
    ...
    def test_compute_chi2_top_feature(self):
        ...
        labels, con_vec, features = chi2_analyzer._preprocess_chi2(
            test_data, self.lang_util
        )
        unigrams, bigrams = chi2_analyzer._compute_chi2_top_feature(
            features, labels, con_vec, "boston", 0.05
        )
        ...
        self.assertEqual(
            unigrams, ["need", "me", "assist", "decid", "help"], "chi2 analyzer fail"
        )
        test_bigrams = [
            "assist me",
            "you assist",
    ...
    def test_get_chi2_analysis(self):
        ...
        unigram_intent_dict, bigram_intent_dict = chi2_analyzer.get_chi2_analysis(
            test_data, significance_level=0.05, lang_util=self.lang_util
        )
        self.assertEqual(len(unigram_intent_dict), 0, "chi2 analyzer fail")

        unigram_intent_dict, bigram_intent_dict = chi2_analyzer.get_chi2_analysis(
            self.workspace_df, significance_level=0.05, lang_util=self.lang_util
        )
        list_of_intent_list = list(unigram_intent_dict.values())
        one_bigram_set = list(bigram_intent_dict.keys())[0]
        self.assertEqual(
            all(len(intents) >= 1 for intents in list_of_intent_list),
            True,
            "chi2 analyzer fail",
        )
        self.assertEqual(
            all(len(item.split(" ")) == 2 for item in one_bigram_set),
            True,
            "chi2 analyzer fail",
        )
    ...
    def test_get_confusing_keyterms(self):
        ...
        ambiguous_data_frame = chi2_analyzer.get_confusing_key_terms(
            unigram_intent_dict
        )
        self.assertTrue(
            str(ambiguous_data_frame.iloc[0, 0]) == "<intent1, intent2>"
            or str(ambiguous_data_frame.iloc[0, 0]) == "<intent2, intent1>",
            "chi2 analyzer fail",
        )
        self.assertEqual(
            "a" in list(ambiguous_data_frame["Terms"]), True, "chi2 analyzer fail"
        )
