...
class TestKeywordAnalyzer(unittest.TestCase):
    ...
    def test_get_counts_per_label(self):
        counts = keyword_analyzer._get_counts_per_label(self.test_data, self.lang_util)
        self.assertEqual(
            ("boston", "Celtics") in counts.index.tolist(),
            True,
            "Key word analyzer test fails",
        )
        self.assertEqual(
            ("nyc", "coast") in counts.index.tolist(),
            True,
            "Key word analyzer test fails",
        )
        self.assertEqual(
            ("boston", "is") in counts.index.tolist(),
            False,
            "Key word analyzer test fails",
        )
    ...
    def test_preprocess_for_heat_map(self):
        counts, top_counts = keyword_analyzer._preprocess_for_heat_map(
            self.workspace_df,
            label_for_display=30,
            max_token_display=30,
            class_list=None,
