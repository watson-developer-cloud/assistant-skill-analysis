...
class TestDivergenceAnalyzer(unittest.TestCase):
    ...
    def test_train_test_vocab_difference(self):
        train_vocab, test_vocab = divergence_analyzer._train_test_vocab_difference(
            self.train_set_pd, self.test_set_pd
        )

        self.assertEqual(
            train_vocab,
            set(["boston", "is", "close", "to", "new", "york"]),
            "train test vocab difference test fail",
        )

    def test_train_test_uttterance_length_difference(self):
        temp_df = divergence_analyzer._train_test_utterance_length_difference(
            self.train_set_pd, self.test_set_pd
        )

        self.assertEqual(
            temp_df.iloc[0]["Absolute Difference"],
            1.5,
            "train test utterance length differene test fail",
        )

    def test_train_test_label_difference(self):
        # Test 1
        percentage_dict1 = {"Intent1": 0.5, "Intent2": 0.5}
        percentage_dict2 = {"Intent1": 0.5, "Intent2": 0.5}

        (
            missing_labels,
            difference_dict,
            js_distance,
        ) = divergence_analyzer._train_test_label_difference(
            percentage_dict1, percentage_dict2
        )
        ...
        self.assertEqual(
            difference_dict["Intent1"], [50, 50, 0], "train test difference test fail"
        )
        ...
        self.assertEqual(
            difference_dict["Intent1"], [100, 100, 0], "train test difference test fail"
        )
        ...
        self.assertEqual(
            difference_dict["Intent1"], [100, 0, 100], "train test difference test fail"
        )
        self.assertEqual(
            difference_dict["Intent2"], [0, 100, 100], "train test difference test fail"
        )
